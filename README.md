...
- **WinTAK → Meshtastic (generic CoT / PLI):** only true PLI position events (`a-f-G-U-C`) are encoded as Meshtastic `ATAK_PLUGIN` PLI packets on port `72`. Marker and other generic CoT events are now kept separate from PLI, preserve their original `uid` / `type` / `how` / `point` semantics, and prefer compact Meshtastic `TAKPacket detail=7` packets on the same ATAK plugin port before using oversized-event fallback via `ATAK_FORWARDER` on port `257` with zlib-compressed CoT XML and automatic fragment reassembly on receipt. Incoming Meshtastic `ATAK_PLUGIN` PLI and `detail=7` marker packets are reconstructed back into CoT and forwarded to TAK. Binary mesh packets are broadcast using the Meshtastic data-port defaults for better Python API compatibility; legacy `COTM:` short-text fragments on main channel `0` remain only as the final emergency fallback.
