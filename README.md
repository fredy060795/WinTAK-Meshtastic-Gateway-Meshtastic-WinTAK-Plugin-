...
- **WinTAK → Meshtastic (generic CoT):** any other CoT event received on the same UDP listener is sent as a Meshtastic `ATAK_FORWARDER` data packet with zlib-compressed CoT XML on main channel `0`, matching the official Meshtastic ATAK plugin expectations for future ATAK/iTAK marker compatibility. For backward compatibility, the gateway can still receive and reassemble older legacy `COTM:` text fragments.
