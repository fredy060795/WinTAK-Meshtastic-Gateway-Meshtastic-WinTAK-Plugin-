...
local_tak_tcp_pong_stale_seconds: 120 # Validity window for local WinTAK TCP pong/keepalive replies
...
- **WinTAK TCP path (LPU5-style):** the gateway now also starts a TCP receiver client that connects to `local_tak_tcp_receiver_host:local_tak_tcp_receiver_port` (defaults `127.0.0.1:8087`) and ingests stream-framed CoT/GeoChat exactly like the working `lpu5-tactical` bridge. Ping/pong keepalives are handled on that client connection before packets are forwarded through the gateway's existing canonical inbound GeoChat/CoT handler. The same active TCP session is now also used in the opposite direction, so mesh-originated CoT is pushed straight back to connected WinTAK/ATAK clients instead of relying only on local UDP delivery.
- **Keepalive tuning:** local WinTAK TCP pong replies now default to a `120` second validity window. If your local `127.0.0.1:8087` WinTAK server still drops the connection, raise `local_tak_tcp_pong_stale_seconds` in `config.yaml` to match a longer WinTAK keepalive cadence.
