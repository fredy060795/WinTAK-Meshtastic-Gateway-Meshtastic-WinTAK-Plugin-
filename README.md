...
- **WinTAK → Meshtastic (generic CoT / PLI):** friendly position CoT events (`a-f-G-U-C`) are now encoded as Meshtastic `ATAK_PLUGIN` PLI packets on port `72`, matching the official ATAK plugin format for contact markers. All other CoT events continue to use `ATAK_FORWARDER` on port `257` with zlib-compressed CoT XML on main channel `0`. For backward compatibility, the gateway can still receive and reassemble older legacy `COTM:` text fragments.
