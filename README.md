...
- **WinTAK → Meshtastic (GeoChat):** configure WinTAK to send outgoing GeoChat CoT via UDP to `local_tak_chat_listen_port` (default `4242`) on the gateway host. By default the UDP listener binds to `0.0.0.0`, so WinTAK can target either `127.0.0.1` or the gateway PC's LAN IP. The gateway now forwards normal and 1:1 GeoChat as Meshtastic `ATAK_PLUGIN` chat packets on port `72`, so official ATAK/iTAK recipients see the same direct-message destination metadata instead of a downgraded plain text broadcast; it only falls back to plain text when the plugin packet is too large or unsupported by the connected Python API.
...
- **WinTAK → Meshtastic (generic CoT / PLI):** only true PLI position events (`a-f-G-U-C`) are encoded as Meshtastic `ATAK_PLUGIN` PLI packets on port `72`. Marker and other generic CoT events are now kept separate from PLI, preserve their original `uid` / `type` / `how` / `point` semantics, and prefer compact Meshtastic `TAKPacket detail=7` packets on the same ATAK plugin port before using oversized-event fallback via `ATAK_FORWARDER` on port `257` with zlib-compressed CoT XML and automatic fragment reassembly on receipt. Incoming Meshtastic `ATAK_PLUGIN` PLI and `detail=7` marker packets are reconstructed back into CoT and forwarded to TAK; detail=7 packets are no longer allowed to fall through into the PLI path when decode fails, and `ATAK_FORWARDER` decode now also tolerates Meshtastic transfer-type prefixes (`0x00`/`'0'`) used by official plugin transports. Binary mesh packets are broadcast using the Meshtastic data-port defaults for better Python API compatibility; legacy `COTM:` short-text fragments on main channel `0` remain only as the final emergency fallback.
