...
| **CoT over Mesh** | Friendly position markers use official Meshtastic `ATAK_PLUGIN` PLI packets and other WinTAK CoT/marker events now prefer official `ATAK_PLUGIN` `detail=7` payloads; only oversized events fall back to mesh-safe `COTM:` fragments and then zlib-compressed `ATAK_FORWARDER`. |
